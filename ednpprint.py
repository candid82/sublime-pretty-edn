...
def plugin_loaded():
    global settings
    settings = sublime.load_settings('Ednpprint.sublime-settings')
...
class Error(object):
    ...
    # <joker.core>:2123:13: Eval error: <>:2:11: Read error: Map literal must contain an even number of forms
    line_re = re.compile(r'.*Eval error: <>:(\d+):(\d+):\s+(.*)\Z')
...
class Formatter(object):
    ...
    def _clear_errors(self):
        self.view.set_status('ednpprint', '')
        self.view.erase_regions('ednpprint')

    def _hide_error_panel(self):
        self.window.run_command('hide_panel', {'panel': 'output.ednpprint'})

    def _show_errors(self, errors, return_code, cmd):
        self.view.set_status('ednpprint', '{} failed with return code {}'.format(
    ...
    def _show_error_regions(self, errors):
        self.view.add_regions(
            'ednpprint', [e.region for e in errors], 'invalid.illegal', 'dot',
            (sublime.DRAW_NO_FILL | sublime.DRAW_NO_OUTLINE |
             sublime.DRAW_SQUIGGLY_UNDERLINE))

    def _show_error_panel(self, errors):
        characters = '\n'.join([e.text for e in errors])
        p = self.window.create_output_panel('ednpprint')
        p.set_scratch(True)
...
class EdnpprintCommand(sublime_plugin.TextCommand):
...
class EdnPpprintListener(sublime_plugin.EventListener):
